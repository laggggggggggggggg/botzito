...
class RedeTugaBot(commands.Bot):
    ...
    async def setup_hook(self) -> None:
        ...
        cogs_ordenados = ["setup", "geral", "editar", "regras", "boas_vindas", "tickets",
                          "anuncios", "falar", "embed_builder", "sla", "giveaways", "staff"]
        ...
        # Sincroniza slash commands APENAS para o servidor permitido
        # Isto garante que os comandos NÃO aparecem em outros servidores
        try:
            guild_permitida = discord.Object(id=GUILD_ID_PERMITIDO)
            # Copia todos os comandos para a guild permitida
            self.tree.copy_global_to(guild=guild_permitida)
            synced = await self.tree.sync(guild=guild_permitida)
            log.info("🔄 %d slash commands sincronizados (guild %d)", len(synced), GUILD_ID_PERMITIDO)

            # LIMPA comandos globais — remove de todos os outros servidores
            # Isto faz com que os comandos desapareçam de servidores não permitidos
            ...
            if global_commands:
                for cmd in global_commands:
                    try:
                        ...
                        pass
                # Sincroniza lista vazia global = remove todos os comandos globais
                await self.tree.sync()
                log.info("🧹 %d comandos globais removidos (limpeza)", len(global_commands))
        except Exception as e:
...
# ─────────────────────────────────────────────────────────────────────────────
# BLOQUEIO DE SERVIDOR — interaction_check (bloqueia ANTES de processar)
# ─────────────────────────────────────────────────────────────────────────────
@bot.tree.interaction_check
async def global_interaction_check(interaction: discord.Interaction) -> bool:
    """Bloqueia TODAS as interações em guilds não-permitidas — ANTES do processamento.

    Isto substitui on_interaction (que disparava DEPOIS do comando já processado).
    interaction_check é chamado ANTES do callback — return False bloqueia a execução.
    """
    if interaction.guild is not None and interaction.guild.id != GUILD_ID_PERMITIDO:
        try:
            ...
            pass
        return False  # ← BLOQUEIA a execução do comando/botão
    return True
...
async def cmd_ajuda(interaction: discord.Interaction) -> None:
    ...
    if is_admin:
        ...
        embed.add_field(
            name="👥 `/staff`",
            value="Gere quem tem acesso aos comandos do Tuguinha (lista, adicionar, remover).",
            inline=False,
        )
