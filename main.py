...
async def on_ready() -> None:
    ...
    try:
        # Rich presence — "Jogando Rede Tuga"
        await bot.change_presence(
            status=discord.Status.online,
            activity=discord.Game(name="Rede Tuga"),
        )
    except Exception as e:
...
async def cmd_criador(interaction: discord.Interaction) -> None:
    ...
    embed.set_thumbnail(
        url=bot.user.display_avatar.url if bot.user else discord.Embed.Empty
    )
    embed.set_footer(text="Tuguinha • Feito com 🤍 pelo GOAT Ricardo 🇵🇹")
