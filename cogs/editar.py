...
from utils import embed_aviso, embed_erro, embed_sucesso, e_admin, log_evento, mention_canal_regras, mention_canal_tickets
...
class EditarMensagemModal(discord.ui.Modal):
    ...
    async def on_submit(self, interaction: discord.Interaction) -> None:
        ...
        preview = preview.replace("{regras}", mention_canal_regras(self.cfg))
        preview = preview.replace("{tickets}", mention_canal_tickets(self.cfg))
        ...
        for chave, label, emoji, attr in [
            ...
            def make_callback(attr=attr, label=label):
                async def cb(interaction: discord.Interaction) -> None:
                    if not await check_admin_and_respond(interaction):
                        return
                    # C3: Restringe mudança de cargo_admin a Discord admins
                    if attr == "cargo_admin" and not interaction.user.guild_permissions.administrator:
                        await interaction.response.send_message(
                            embed=embed_erro(
                                "🚫 Sem permissão",
                                "Apenas **administradores do Discord** podem alterar o cargo de Admin.",
