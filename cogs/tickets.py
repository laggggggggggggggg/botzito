...
# ─────────────────────────────────────────────────────────────────────────────
class MotivoFechoModal(discord.ui.Modal):
    """Modal que pede o motivo de encerramento do ticket."""

    def __init__(self, cog: "TicketsCog", quem_pediu_id: int) -> None:
        super().__init__(title="🔒 Fechar Ticket", timeout=120)
        self.cog = cog
        self.quem_pediu_id = quem_pediu_id

        self.motivo = discord.ui.TextInput(
            label="Motivo de encerramento",
            placeholder="Ex: Problema resolvido / Sem resposta do membro / etc.",
    ...
    async def on_submit(self, interaction: discord.Interaction) -> None:
        # Verifica permissões
        if interaction.user.id != self.quem_pediu_id and not e_staff(interaction.user):
            await interaction.response.send_message(
                "🚫 Não foste tu que iniciou o fecho.", ephemeral=True
            )
            return
        motivo = self.motivo.value.strip() or "Não especificado"
        await interaction.response.edit_message(view=None)
        await self.cog._executar_fecho(interaction, motivo)


class ConfirmarFechoView(discord.ui.View):
    """View que pergunta se quer fechar — abre modal de motivo."""

    def __init__(self, quem_pediu_id: int = 0) -> None:
        super().__init__(timeout=30)
        self.quem_pediu_id = quem_pediu_id

    async def _check(self, interaction: discord.Interaction) -> bool:
        if self.quem_pediu_id and interaction.user.id != self.quem_pediu_id and not e_staff(interaction.user):
            await interaction.response.send_message(
                "🚫 Apenas quem iniciou o fecho ou a staff pode confirmar.",
                ephemeral=True,
            )
            return False
        return True

    @discord.ui.button(label="Confirmar fecho", emoji="✅", style=discord.ButtonStyle.danger)
    async def confirmar(self, interaction: discord.Interaction, button: discord.ui.Button) -> None:
        ...
        await interaction.response.send_modal(
            MotivoFechoModal(self.cog if hasattr(self, 'cog') else _get_tickets_cog(interaction.client), self.quem_pediu_id)
        )
    ...
    async def cancelar(self, interaction: discord.Interaction, button: discord.ui.Button) -> None:
        if not await self._check(interaction):
            return
        try:
            await interaction.response.edit_message(content="❌ Fecho cancelado.", view=None)
        except discord.HTTPException:
            pass


def _get_tickets_cog(bot: commands.Bot) -> "TicketsCog":
    """Helper para obter a TicketsCog do bot."""
...
class TicketsCog(commands.Cog):
    ...
    async def criar_ticket(
        ...
        # Cargo de suporte fixo da Rede Tuga
        CARGO_SUPORTE_ID = 1489030726954385533

        for cargo_id in (CARGO_SUPORTE_ID, cfg.cargo_ticket_staff, cfg.cargo_staff):
            if not cargo_id:
                continue
    ...
    async def fechar_ticket(self, interaction: discord.Interaction) -> None:
        """Inicia o processo de fecho — mostra confirmação."""
        ...
        view_confirmar = ConfirmarFechoView(interaction.user.id)
        # Passa a referência à cog para o modal funcionar
        view_confirmar.cog = self
        await interaction.response.send_message(
            embed=embed_aviso(
                f"{Emojis.FECHAR} Fechar ticket?",
                f"{interaction.user.mention}, queres fechar este ticket?\n\n"
                f"Clica em **Confirmar fecho** para continuar. Vai pedir um motivo.",
                cfg.nome_servidor,
            ),
            view=view_confirmar,
        )

    async def _executar_fecho(self, interaction: discord.Interaction, motivo: str) -> None:
        """Executa o fecho do ticket com motivo."""
        canal = interaction.channel
        if not isinstance(canal, discord.TextChannel):
            return
        ...
        # 1. Lê todas as mensagens do ticket para a DM
        mensagens = []
        try:
            async for msg in canal.history(limit=100, oldest_first=True):
                ...
                conteudo = msg.clean_content or ""
                # Se a mensagem tem embeds, extrai descrição
                for emb in msg.embeds:
                    if emb.description:
                        conteudo += f"\n{emb.description}" if conteudo else emb.description
                if conteudo.strip():
                    mensagens.append((timestamp, autor, conteudo[:500]))
        except discord.HTTPException:
            pass

        # 2. Envia transcript para o canal de transcripts (com motivo)
        await self._gerar_e_enviar_transcript(
            canal, interaction.user, ticket_encontrado, cfg.nome_servidor,
            interaction=interaction, motivo=motivo,
        )

        # 3. Envia DM ao dono com a conversa formatada
        await self._enviar_dm_conversa(ticket_encontrado, cfg, mensagens, motivo, interaction.user)

        # 4. Envia DM de feedback (estrelas)
        ...
        try:
            await canal.send(
                embed=embed_aviso(
                    "🔒 A fechar...",
                    f"Este canal será apagado em 3 segundos.\n**Motivo:** {motivo}",
                    cfg.nome_servidor,
                )
            )
        ...
        if canal_apagado:
            async with self._ticket_lock:
                ...
                ticket_encontrado["motivo_fecho"] = motivo
                estado["tickets"][ticket_id_encontrado] = ticket_encontrado

                # Limpa tickets fechados há mais de 30 dias
            ...
            try:
                await canal.send(
                    embed=embed_erro(
                        ...
                        "Não foi possível apagar o canal. Tenta novamente.",
                        cfg.nome_servidor,
                    )
                )
            except discord.HTTPException:
    ...
    async def _enviar_dm_conversa(
        self, ticket_info: dict, cfg, mensagens: list, motivo: str, quem_fechou: discord.abc.User
    ) -> None:
        """Envia DM ao dono com a conversa formatada (não como ficheiro)."""
        ...
        categoria = ticket_info.get("categoria_label", "?")

        # Embed de cabeçalho
        embed_header = discord.Embed(
            title=f"🔒 O teu ticket foi fechado",
            description=(
                f"**Ticket:** `{ticket_id}`\n"
                f"**Categoria:** {categoria}\n"
                f"**Fechado por:** {quem_fechou.mention}\n"
                f"**Motivo:** {motivo}\n\n"
                f"Aqui está o resumo da conversa:"
            ),
            color=Cores.VERMELHO,
            timestamp=datetime.now(timezone.utc),
        )
        embed_header.set_footer(text=f"{cfg.nome_servidor} • Ticket {ticket_id}")

        try:
            await user.send(embed=embed_header)
        except (discord.Forbidden, discord.HTTPException):
            return  # DM fechada — não podemos fazer nada

        # Envia mensagens em blocos (Discord limite 2000 chars por mensagem)
        bloco = ""
        for timestamp, autor, conteudo in mensagens:
            linha = f"**[{timestamp}] {autor}:**\n{conteudo}\n\n"
            if len(bloco) + len(linha) > 1900:
                # Envia bloco atual e começa novo
                try:
                    await user.send(bloco)
                except discord.HTTPException:
                    pass
                bloco = linha
            else:
        ...
        embed_final.set_footer(text="Tuguinha • Rede Tuga 🇵🇹")
        view = FeedbackStarsView(user_id, cfg.nome_servidor)
        try:
            await user.send(embed=embed_final, view=view)
        except discord.HTTPException:
            pass
    ...
    async def _gerar_e_enviar_transcript(
        ...
        linhas.append(f"Motivo de encerramento: {motivo}")
        linhas.append("─" * 64)
        linhas.append("")
