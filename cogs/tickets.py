...
class SelecaoCategoriaView(discord.ui.View):
    ...
    async def _on_select(self, interaction: discord.Interaction) -> None:
        ...
        cog = self.cog
        # ── Cooldown: 1 ticket a cada 60 segundos por utilizador por guild ──
        agora = datetime.now(timezone.utc)
        chave_cooldown = (interaction.guild.id, interaction.user.id)
        ultimo = cog._cooldowns.get(chave_cooldown)
        ...
        async with cog._ticket_lock:
            ...
            cog._cooldowns[chave_cooldown] = agora
...
class TicketsCog(commands.Cog):
    ...
    async def _gerar_e_enviar_transcript(
        ...
        # Envia ao canal_transcript configurado (PRIORITÁRIO — sempre que possível)
        ...
        # Fallback: canal de logs se transcript não estiver configurado
