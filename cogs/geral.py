...
class PainelBoasVindasView(discord.ui.View):
    ...
    def __init__(self, cog: "GeralCog", guild: discord.Guild) -> None:
        ...
        # Remove o canal atual da lista para evitar opções duplicadas
        if self.cfg.canal_bem_vindas:
            canais = [c for c in canais if c.id != self.cfg.canal_bem_vindas]

        options_bv = []
        # Adiciona o canal atual primeiro (se configurado)
        if self.cfg.canal_bem_vindas:
