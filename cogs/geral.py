...
class PainelBoasVindasView(discord.ui.View):
    ...
    def __init__(self, cog: "GeralCog", guild: discord.Guild) -> None:
        ...
        canais = list(guild.text_channels[:24])
        options_bv = [
    ...
    async def _on_canal(self, interaction: discord.Interaction) -> None:
        ...
        if valor == "off":
            self.cfg.boas_vindas_ativas = False
            ...
            return
        canal_id = int(valor)
        self.cfg.canal_bem_vindas = canal_id
        self.cfg.boas_vindas_ativas = True
        save_config(self.cfg)
    ...
    async def _on_editar_msg(self, interaction: discord.Interaction) -> None:
        if not e_admin(interaction.user):
            await interaction.response.send_message("🚫 Apenas admins.", ephemeral=True)
            return
        await interaction.response.send_modal(EditarMensagemBVModal(self.cog, self.cfg))

    async def _on_testar(self, interaction: discord.Interaction) -> None:
        if not e_admin(interaction.user):
            await interaction.response.send_message("🚫 Apenas admins.", ephemeral=True)
            return
        if not self.cfg.canal_bem_vindas:
            await interaction.response.send_message(
                embed=embed_erro("🚫 Sem canal", "Configura primeiro um canal de boas-vindas.", self.cfg.nome_servidor),
                ephemeral=True,
            )
            return
        canal = self.guild.get_channel(self.cfg.canal_bem_vindas)
        if canal is None:
            await interaction.response.send_message(
                embed=embed_erro("🚫 Canal não encontrado", "O canal configurado já não existe.", self.cfg.nome_servidor),
                ephemeral=True,
            )
            return

        # Simula boas-vindas do próprio utilizador
        msg = self.cfg.get_msg_bem_vindo()
        msg = msg.replace("{user}", interaction.user.mention)
        msg = msg.replace("{count}", str(self.guild.member_count or 0))
        msg = msg.replace("{regras}", f"<#{self.cfg.canal_regras}>" if self.cfg.canal_regras else "#regras")
        msg = msg.replace("{tickets}", f"<#{self.cfg.canal_tickets}>" if self.cfg.canal_tickets else "#tickets")

        embed = discord.Embed(
            title=f"{Emojis.BEM_VINDO} Bem-vindo à {self.cfg.nome_servidor}! (TESTE)",
            description=msg,
            color=Cores.VERMELHO,
        )
        embed.set_thumbnail(url=interaction.user.display_avatar.url)
        embed.set_footer(text=f"🧪 Teste • {self.cfg.nome_servidor}")
...
class EditarMensagemBVModal(discord.ui.Modal):
    def __init__(self, cog: "GeralCog", cfg) -> None:
        ...
        self.mensagem = discord.ui.TextInput(
            label="Mensagem ({user}, {count}, {regras}, {tickets})",
            placeholder="Deixa vazio para usar o template padrão...",
