...
from utils import embed_base, embed_erro, embed_sucesso, log_evento


# ─────────────────────────────────────────────────────────────────────────────
# View do botão de verificação — persistente
# ─────────────────────────────────────────────────────────────────────────────
class VerificacaoView(discord.ui.View):
    ...
    async def verificar(
        ...
        # Atribui cargos configurados
        cargos_adicionados: list[str] = []
        for cargo_id in (config.cargo_verificado, config.cargo_membro):
            if not cargo_id:
                continue
        ...
        if cargos_adicionados:
            await interaction.response.send_message(
                embed=embed_sucesso(
                    ...
                    f"Bem-vindo à **{config.nome_servidor}**! 🇵🇹 Já podes ver todos os canais.",
                ),
                ephemeral=True,
            )
            await log_evento(
...
class BoasVindasCog(commands.Cog):
    ...
    def __init__(self, bot: commands.Bot) -> None:
        self.bot = bot
        # Regista a view persistente para os botões continuarem a funcionar após restart
    ...
    async def on_member_join(self, member: discord.Member) -> None:
        ...
        # 1. Mensagem no canal de boas-vindas
        await self._enviar_boas_vindas_canal(member)

        # 2. DM com regras resumidas
        await self._enviar_dm(member)
    ...
    async def on_member_remove(self, member: discord.Member) -> None:
        """Acionado quando um membro sai do servidor."""
        if member.bot:
            return
        await log_evento(
            self.bot,
    ...
    # Helpers
    # # ─────────────────────────────────────────────────────────────────────────
    async def _enviar_boas_vindas_canal(self, member: discord.Member) -> None:
        if not config.canal_bem_vindas:
            return
        canal = self.bot.get_channel(config.canal_bem_vindas)
        if canal is None:
            return
        ...
        embed = discord.Embed(
            ...
            description=(
                ...
                f"Se precisares de ajuda, abre um ticket no canal #tickets. Boa estadia! 💪"
            ),
        ...
        embed.set_footer(text=f"Membro #{membros:04d} • {config.nome_servidor}")

        view = VerificacaoView()
        try:
            await canal.send(content=member.mention, embed=embed, view=view, allowed_mentions=discord.AllowedMentions(users=True))
        except discord.HTTPException:
            pass

    async def _enviar_dm(self, member: discord.Member) -> None:
        embed = discord.Embed(
            title=f"{Emojis.REGRAS} Bem-vindo à {config.nome_servidor}! 🇵🇹",
            description=(
        ...
        embed.add_field(
            name="✅ Próximo passo",
            value=(
                f"Vai ao canal <#{config.canal_bem_vindas}> e clica no botão **Verificar-me** "
        ...
        try:
            ...
            # DM fechada — não é problema, a verificação também está no canal
            pass

    # ─────────────────────────────────────────────────────────────────────────
    # Comando de painel de boas-vindas (staff)
    # ─────────────────────────────────────────────────────────────────────────
    ...
    async def painel_boas_vindas(
        ...
        alvo = canal or (self.bot.get_channel(config.canal_bem_vindas) if config.canal_bem_vindas else interaction.channel)
        if alvo is None:
            await interaction.response.send_message(
                embed=embed_erro("Sem canal", "Indica um canal válido."),
                ephemeral=True,
            )
            return
        ...
        embed = discord.Embed(
            ...
            description=(
                f"Olá! Para teres acesso a todos os canais da **{config.nome_servidor}**, "
                ...
                f"Se tiveres problemas, fala com a staff em <#{config.canal_tickets if hasattr(config, 'canal_tickets') else 0}>."
            ),
            color=Cores.VERDE,
        )
        embed.set_thumbnail(url=interaction.guild.icon.url if interaction.guild.icon else discord.Embed.Empty)
        embed.set_footer(text=f"{config.nome_servidor} • Sistema de Verificação")
